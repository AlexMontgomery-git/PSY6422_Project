...
## Current Study
As previously mentioned, how often specific heroes are picked partly depends on the **skill level of players**. The current study shall set out to use visualisations to explore the relationship between **hero pick rate** and **skill level (division)**. The visualisations should be able to identify which specific heroes are picked more in lower ranks than higher ranks and vice versa, as well as to be able to identify heroes whose pick rate does not significantly vary based on skill level. 
...
Visualisation 1 allows comparison of the pick rates across all divisions, however, there are some limitations to the design. Firstly, although the cumulative pick rate of a hero demonstrates which heroes are picked more overall, it does not represent a real world figure as the overall average pick rate for a hero should be the average across all 7 divisions rather than the sum (as it is represented in the visualisation). A grouped bar chart would have overcome this issue. However, this would include too many bars on the x axis as each hero would have 7 bars, totaling 224. 

A further issue of visualisation 1 is that it is hard to see stand out trends due to the overwhelming amount of data that is being shown. In hindsight, the inclusion of all the data may have been more detrimental than useful in respect of the readability of the visualisation. Upon closer inspection it is still possible to identify heroes with significantly varying pick rates across divisions, such as Moira being picked far more in lower divisions compared to higher ones. However, it would take some time to examine the whole visualisation to pick out all the significant trends. 

This issue will be addressed in the creation of visualisation 2. Less data will be represented in the visualisation but with the important trends still being identifiable and much more easily spotted, with less time needed to examine the visualisation.
...
Overall, visualisation 2 makes it far easier to spot trends of heroes that are picked significantly more or less in higher or lower divisions. The steeper the slope of the line, the bigger the difference in pick rate there is from the lowest division to the highest division. It is relatively easy to pick out the steepest slopes to identify the heroes whose pick rate is the most affected by skill level (division). The opposite is true when identifying heroes whose pick rate is least affected by skill level as identified by the flattest lines. Identifying specific heroes is made easier by the choice of colour of the lines. Each hero's line is represented by the colour that is associated with that specific hero. As the target demographic reading this visualisation are individuals who are interested in Overwatch, it is safe to assume that they will know what colours are associated with each hero. To make the visualisation more readable, the lines were made slightly transparent to overcome the issue of many lines crossing one another. The colours in the code were assigned in alphabetical order of the heroes, meaning the use of an updated dataset (in which the heroes will likely be in a different order) will still work whilst keeping the colours for each hero correct.
...
The repo for this project (including the markdown file, original data and image files) are available online via github: https://alexmontgomery-git.github.io/PSY6422_Project/
